...
valid_services = ["api", "postgress"]

# Buildable services
buildable_services = [ "api", "pipelines" ]

# Startable service
startable_services = [ "dashboard", "api" ]
...
def start(services):
    ...
    for service in services:
        if service == "dashboard":
            ...
            os.system("minikube service registries-rabbitmq-service")
        elif service == "api":
            os.system("minikube service --url registries-api-service")
...
def build(services):
    ...
    for service in services:
        if service in buildable_services:
            os.system(f"docker build -t tapis/registries-{service}:dev -f src/{service}/src/Dockerfile .")
...
def rebuild(services):
    ...
    for service in services:
        if service in buildable_services:
            os.system(f"docker build -t tapis/registries-{service}:dev -f ./src/{service}/src/Dockerfile . && docker push tapis/registries-{service}:dev")
